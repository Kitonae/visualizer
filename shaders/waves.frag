/* Water waves shader - converted from Shadertoy to LÖVE2D
   Original by afl_ext 2017-2024, MIT License
...
#define NormalizedMouse (mouse.xy / resolution.xy) // normalize mouse coords
...
vec3 getRay(vec2 fragCoord) {
  ...
  // Rotate camera upside down by adding 180 degrees (PI radians) around Z axis
  return createRotationMatrixAxisAngle(vec3(0.0, 0.0, 1.0), 3.14159) *
    createRotationMatrixAxisAngle(vec3(0.0, -1.0, 0.0), 3.0 * ((NormalizedMouse.x + 0.5) * 2.0 - 1.0)) 
...
// Main effect function for Love2D
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
  // get the ray
  vec3 ray = getRay(screen_coords);
  if(ray.y >= 0.0) {
