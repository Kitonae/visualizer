...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) {
    ...
    vec2 uv = (screen_coords - 0.5 * r) / r.y;

    float t = time;
    float z = fract(dot(screen_coords, sin(screen_coords))) - 0.5;
    vec4 col = vec4(0.0);
    vec4 p;

    for (float i = 0.0; i < 77.0; i++) {
        // Ray direction
        p = vec4(z * normalize(vec3(screen_coords - 0.7 * r, r.y)), 0.1 * t);
        p.z += t;
    ...
    return vec4(tanh_vec3(col.rgb / 2e4), 1.0);
}
